#ifndef BEAM_SEARCH_DECODER_WITH_WORD_PROBS_H
#define BEAM_SEARCH_DECODER_WITH_WORD_PROBS_H
 ...
 * Veritone:
 *
 *  This mirrors the CTC beam search decoder in Tensorflow (tensorflow/core/util/ctc/ctc_beam_search.h).
 *  The reason for having our own version here is that to capture and output word log probabilities, we need to modify
 *  the GetTopPaths function. This function examines the beam_width_ and leaves_ member variables, both of which are
 *  declared private in the original Tensorflow class.
 *
 *  As a result, since we can't access these variables via subclassing, we instead copy its functionality here and
 *  make the required changes in the TopPaths function to augment its functionality with word level log probabilities.
 */
namespace tensorflow {
    namespace ctc {
        ...
        class BeamSearchDecoderWithWordProbs : public CTCDecoder {
            // Beam Search
            //
        ...
        public:
            ...
            BeamSearchDecoderWithWordProbs(int num_classes, int beam_width,
            ...
            ~BeamSearchDecoderWithWordProbs() override {}
            ...
            Status TopPaths(
                    ...
                    std::vector<std::vector<float>>* word_log_probs,
                    bool merge_repeated
            ) const;
        ...
        private:
            ...
            TF_DISALLOW_COPY_AND_ASSIGN(BeamSearchDecoderWithWordProbs);
        };

        template <typename CTCBeamState, typename CTCBeamComparer>
        Status BeamSearchDecoderWithWordProbs<CTCBeamState, CTCBeamComparer>::Decode(
            ...
            std::vector<std::vector<float>> beam_word_log_probabilities;
            ...
            for (int b = 0; b < batch_size_; ++b) {
                ...
                Status status =
                        TopPaths(top_n, &beams, &beam_log_probabilities, &beam_word_log_probabilities, merge_repeated_);
                if (!status.ok()) {
                    return status;
                }
        ...
        template <typename CTCBeamState, typename CTCBeamComparer>
        template <typename Vector>
        float BeamSearchDecoderWithWordProbs<CTCBeamState, CTCBeamComparer>::GetTopK(
                const int K,
                const Vector& input,
                std::vector<float>* top_k_logits,
                std::vector<int>* top_k_indices
        ...
        template <typename CTCBeamState, typename CTCBeamComparer>
        template <typename Vector>
        void BeamSearchDecoderWithWordProbs<CTCBeamState, CTCBeamComparer>::Step(
        ...
        template <typename CTCBeamState, typename CTCBeamComparer>
        void BeamSearchDecoderWithWordProbs<CTCBeamState, CTCBeamComparer>::Reset() {
            leaves_.Reset();
        ...
        template <typename CTCBeamState, typename CTCBeamComparer>
        Status BeamSearchDecoderWithWordProbs<CTCBeamState, CTCBeamComparer>::TopPaths(
                int n,
                std::vector<std::vector<int>>* paths,
                std::vector<float>* log_probs,
                std::vector<std::vector<float>>* word_log_probs,
            ...
            CHECK_NOTNULL(word_log_probs)->clear();
            ...
            for (int i = 0; i < n; ++i) {
                ...
                KenLMBeamState state = e->state;
                paths->push_back(e->LabelSeq(merge_repeated));
                log_probs->push_back(e->newp.total);
                word_log_probs->push_back(state.prev_word_probs);

                LOG(INFO) << "paths size: " << e->LabelSeq(merge_repeated).size();
                LOG(INFO) << "word log probs size: " << state.prev_word_probs.size();
            }
            return Status::OK();
        }
    }
...
#endif //BEAM_SEARCH_DECODER_WITH_WORD_PROBS_H